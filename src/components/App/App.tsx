...
import "./App.css";
// ИМПОРТ: Импортируем обе функции API, включая fetchMovieDetails
...
// ИМПОРТ: Компонент модального окна
import MovieModal from "../MovieModal/MovieModal";
// ИМПОРТ: Импортируем MovieDetails для корректной типизации модалки
...
export default function App() {
  ...
  const [error, setError] = useState<string | null>(null); // Для хранения ошибок // ИЗМЕНЕНИЕ: Используем MovieDetails для выбранного фильма
  const [selectedMovie, setSelectedMovie] = useState<MovieDetails | null>(null); // Для модального окна // НОВОЕ: Состояние для индикации загрузки деталей фильма в модалке
  ...
  const handleMovieSelect = async (movieId: number) => {
    ...
    setError(null);
    setIsModalLoading(true); // Начинаем загрузку деталей
    ...
    <div style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ padding: "20px" }}>
        ...
        {!isLoading && !error && movies.length === 0 && (
          <p style={{ textAlign: "center" }}>
            Введите ключевое слово для поиска фильмов.{" "}
          </p>
        )}
      </div>
